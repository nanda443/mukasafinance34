...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <button class="btn btn-success float-right" data-toggle="modal" data-target="#createSiswaModal">
...
$(document).ready(function() {
    ...
    var table = $('#siswaTable').DataTable({
        ...
        serverSide: true,
        ajax: {
             url: "{{ route('bendahara.data-siswa.index') }}",
            type: "GET",
            data: function (d) {
    ...
    $('#createSiswaForm').submit(function(e) {
        ...
        $.ajax({
            url: "{{ route('bendahara.data-siswa.store') }}",
            method: 'POST',
            data: formData,
            beforeSend: function() {
    ...
    // Edit siswa - Load data
    $(document).on('click', '.edit-siswa', function() {
        var id = $(this).data('id');
        
        var url = "{{ route('bendahara.data-siswa.edit', ':id') }}";
    ...
    $('#editSiswaForm').submit(function(e) {
        ...
        var url = "{{ route('bendahara.data-siswa.update', ':id') }}";
        url = url.replace(':id', id);
    ...
    $(document).on('click', '.delete-siswa', function() {
        ...
        var nis = $(this).closest('tr').find('td:eq(2)').text();
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-left">
                    ...
                    <p class="text-danger font-weight-bold">
                        ...
                        Data yang dihapus akan dipindahkan ke arsip dan dapat dipulihkan jika diperlukan.
                    </p>
                </div>
            `,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                var url = "{{ route('bendahara.data-siswa.destroy', ':id') }}";
                url = url.replace(':id', id);
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        var errorMessage = 'Terjadi kesalahan saat menghapus siswa';
